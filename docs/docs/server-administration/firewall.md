...
::: warning
After every edit or update the firewall, OrbixPanel will clear the current iptables unless the rules are added via OrbixPanel and [custom script](#how-can-i-customize-iptables-rules).
:::
...
6. Select the data source by entering one of the following:
   ...
   - You can also use one of OrbixPanel’s included sources.
...
OrbixPanel supports setting custom rules, chains or flags, etc. using script.
