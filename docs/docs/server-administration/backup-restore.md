...
Currently OrbixPanel only support restoring backups made using:
...
For safety reasons, OrbixPanel takes into account 2x the user’s disk usage when creating a backup. Therefore, before starting a backup, we check how much disk usage a user has left. If you encounter this error, you can do one of the following to solve the issue:
...
Once the config has been saved you can setup OrbixPanel with the following command:
...
Since 1.9 we include Rclone by default in OrbixPanel Installation. Run `rclone config` as the `root` user and then select the option `n`. Follow the instruction on the screen, then save when completed.
